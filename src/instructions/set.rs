...
fn jmp(condition: Option<(Flag, bool)>) -> Micro {
  ...
  match condition {
    None => Micro::Code(c),
    Some((flag, value)) => {
      let mut n = vec![Control::new(), Control::new(), Control::new()];
      n[1].ProgramCounter.Count = IncDec::Increment;
      n[2].ProgramCounter.Count = IncDec::Increment;
...
impl Set {
  pub fn new() -> Set {
    let mut instructions = hash_map!{
      ...
      0b00010011 => jmp(Some((Flag::C, true))),  // JMP  C,addr
      0b00010100 => jmp(Some((Flag::V, false))), // JMP NV,addr
      0b00010101 => jmp(Some((Flag::V, true))),  // JMP  V,addr
      0b00010110 => jmp(Some((Flag::S, false))), // JMP  P,addr
      0b00010111 => jmp(Some((Flag::S, true))),  // JMP  N,addr
      ...
      0b00100010 => operation(Register::X, Operation::Compare, Argument::Byte),
      0b00100011 => operation(Register::X, Operation::Compare, Argument::Register(Register::Y)),
